...
//        }
    ...
    public final void doSelfMove(Vec3 vec3) {
        ...
        if (GameProtocol.is26_20orHigher(player.getSession().protocolVersion())) {
    ...
    private void restituteMovementAfterCollisions(final BoarBlockState state, final boolean xCollision, final boolean zCollision, final Vec3 movement) {
        ...
        if (player.verticalCollision) {
            if (player.onGround) {
                restitution = !(-currentMovement.y < player.getEffectiveGravity()) && !player.getFlagTracker().has(EntityFlag.SNEAKING) && !state.getState().is(Blocks.HONEY_BLOCK) ? Math.max(restitution, state.getBlockBounciness()) : 0.0f;
            }
