    ...
    public void applyInput() {
        ...
        if ((sneaking || player.ticksSinceCrawling > 0 || player.getFlagTracker().has(EntityFlag.GLIDING)) && !player.isInLava() && !player.touchingWater) {
            ...
            if (player.ticksSinceCanSlowdown > 2) {
                final Map<BedrockEnchantment, Integer> enchantments = CompensatedInventory.getEnchantments(player.compensatedInventory.armorContainer.get(2).getData());
                if (enchantments.containsKey(BedrockEnchantment.SWIFT_SNEAK)) {
